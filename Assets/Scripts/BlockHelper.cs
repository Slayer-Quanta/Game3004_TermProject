...
public static class BlockHelper
{
    ...
    {
        // Check if the block is solid based on the BlockDataManager dictionary
        // Return false for special cases like air and water
        if (blockType == BlockType.Air || blockType == BlockType.Nothing || blockType == BlockType.Water)
            return false;

        // For other block types, check if they're defined as solid in the BlockDataManager
        if (BlockDataManager.blockTextureDataDictionary.ContainsKey(blockType))
        {
            return BlockDataManager.blockTextureDataDictionary[blockType].isSolid;
        }

        // Default to true for unknown block types
        return true;
    }
